...
- Source d'entrée : `data/hf_activated_params_dataset.csv`; cible : `activated_params`.
- Le fichier observé contient exactement 220 lignes et 20 colonnes; « environ 200 » était une approximation. Aucun `model_id` n'est répété.
...
- Utiliser `prompt-replacement-selection.md` pour assister le tri des 220 modèles avec un chatbot LLM. Vérifier les propositions avant d'appliquer le nettoyage; conserver le journal d'audit séparé.
...
- Faire le split aléatoire après le nettoyage. Les modèles d'une même famille peuvent être répartis entre partitions. Une évaluation par familles absentes peut être ajoutée si le volume le permet, à titre complémentaire.
